...
export function buildShadowOutcomeReport(records, options = {}) {
  ...
  const candidates = records
    ...
    .map((quote) => ({ quote, scan: scansById.get(quote.scanId) }))
    .filter(({ quote, scan }) => (
      scan &&
      scan.gates?.trendPassed === true &&
      quote.costCoverage?.allowed !== false
    ));
  const outcomes = [];
  for (const { quote, scan } of candidates) {
  ...
  return {
    ...
    method: {
      priceSource: "UNDERLYING_SCAN_CLOSE_PROXY",
      costTreatment: "entry all-in cost deducted once",
      executionStatus: "COUNTERFACTUAL_NON_EXECUTING",
      limitation: "Forward scan prices are not executable sell quotes and do not include an exit-path simulation."
    },
